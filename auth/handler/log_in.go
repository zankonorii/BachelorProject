...
func (h NewHandler) LogIn(ctx echo.Context) error {
	...
	if err := h.DB.QueryRow("SELECT * FROM users WHERE name = ?", info.Name).Scan(&user.ID, &user.Name, &user.LastName, &user.JWT, &user.UpdatedAt, &user.CreatedAt); err != nil {
		logrus.Error(err)
		return ctx.JSON(http.StatusUnauthorized, "please login or register")

	}

	if user.JWT != validToken {
		return ctx.JSON(http.StatusUnauthorized, "please login or register")
	}
